...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        # Use separate cache file for YOLOv7 to avoid conflicts with YOLOv9
        cache_path = (p if p.is_file() else Path(self.label_files[0]).parent).with_suffix('.cache_v7')  # cached labels
        if cache_path.is_file():
            try:
                cache, exists = torch.load(cache_path), True  # load
            except Exception as e:
                print(f'WARNING: Cache file {cache_path} corrupted, recreating...')
