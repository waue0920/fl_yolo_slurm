...
def aggregate(client_weights, client_vectors, global_weights, **kwargs):
    ...
    clip_threshold = float(os.environ.get('SERVER_FEDAWA_CLIP_THRESHOLD', 10.0))
    
    num_clients = len(client_weights)
    
    print(f"[FedAWA] Aggregating {num_clients} clients with PCA_DIM={pca_dim}, CLIP_THRESHOLD={clip_threshold}")
    ...
    print(f"[FedAWA] Weight difference statistics:")
    ...
    print(f"[FedAWA] Performing PCA reduction: {Ucli_arr.shape[1]} → {n_components} dimensions")
    
    try:
        Vcli_arr = pca_reduce(Ucli_arr, n_components=n_components, solver=pca_solver)
    except Exception as e:
        print(f"[ERROR] PCA failed: {e}")
        print(f"[FALLBACK] Using simple averaging instead of FedAWA")
        # Fallback to FedAvg
